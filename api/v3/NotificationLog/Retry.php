...
function civicrm_api3_notification_log_retry($params) {
  if (!empty($params['system_log_id'])) {
    ...
    $logEntries = civicrm_api3('system_log', 'get', [
...
function _civicrm_api3_notification_log_process($logEntry) {
  ...
  elseif (substr($logEntry['message'], 0, 34) == 'payment_notification processor_id=') {
    ...
    $processorTypeId = civicrm_api3('PaymentProcessor', 'getvalue', ['id' => $processorId, 'return' => 'payment_processor_type_id']);
    $processorName = civicrm_api3('PaymentProcessorType', 'getvalue', ['id' => $processorTypeId, 'return' => 'name']);
  }
  else {
    throw new CRM_Core_Exception('unsupported processor');
  }
  // Build the parameter array for the IPN class.
  $ipnParams = array_merge(json_decode($logEntry['context'], TRUE), ['receive_date' => $logEntry['timestamp']]);
  if ($processorId) {
